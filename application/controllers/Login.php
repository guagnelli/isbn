...
class Login extends CI_Controller {
    ...
    public function index() {
        ...
        if ($this->input->post()) {
            ...
            if (($this->form_validation->run() == true) && ($token_html == $token_session)) { //Aplicamos validaciones al nombre de usuario, contraseña, captcha; además se verifica que el token obtenido por el formulario sea el mismo que el que se encuentra en sesión
                $nick = $this->input->post('nick', true);
                $passwd = $this->input->post('passwd', true);

                $resultado = $this->lm->validar_usuario($nick, $passwd); ///Verificar contra base de datos
                //echo "<pre>";
                //var_dump($resultado);
                //echo "</pre>";
                if (isset($resultado['login']->cantidad_reg) && $resultado['login']->cantidad_reg == 1) { ///Usuario existe en base de datos
                    ...
                    if (isset($resultado['login']->cantidad_reg) && $resultado['login']->cantidad_reg == 0) {
                        ...
                        $parametros_log['USUARIO_CVE'] = $resultado['login']->usuario_cve;
                        $parametros_log['INICIO_SATISFACTORIO'] = 0;
                        $resultado = $this->lm->set_log_ususario_doc($parametros_log); //ejecuta procedimiento almacenado de log*/
...
//            pr($this->evaluar_rol($roles));
    ...
    private function checkbrute($nick) {
//        $ahora = time(); ///Tiempo actual
        ...
        $numero_intentos_usuario = $this->lm->set_checkbrute_usuario(intval($nick), intval($lapso_intentos));
